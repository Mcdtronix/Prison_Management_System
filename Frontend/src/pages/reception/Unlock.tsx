...
import { Unlock as UnlockIcon, Save, AlertTriangle } from 'lucide-react';
...
export default function Unlock() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await custodyApi.submitUnlock(payload);
      alert('Unlock submitted successfully');
      navigate('/reception/unlock-history');
    } catch (err: any) {
      alert(err.message || 'Failed to submit unlock');
  ...
  if (loading) return <PrisonLayout title="Daily Unlock" description="Loading..."><div className="p-8">Loading configuration...</div></PrisonLayout>;

  const totalUnlock = calculateStationTotal();

  if (yards.length === 0) {
    return (
      <PrisonLayout title="Daily Unlock" description="Record the physical count of inmates currently unlocked.">
  ...
  return (
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div className="flex justify-between items-center bg-gray-900 text-white p-6 rounded-xl shadow-lg">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3">
            <UnlockIcon className="w-8 h-8" /> Daily Unlock
          </h1>
          <p className="text-gray-300 mt-1">Record the physical count of inmates currently unlocked.</p>
        </div>
        <div className="text-right">
          <p className="text-sm text-gray-400">Total Count</p>
          <p className="text-4xl font-mono font-bold text-green-400">{totalUnlock}</p>
        </div>
      </div>
          ...
          return (
            <div key={yard.id} className="bg-white border rounded-xl overflow-hidden shadow-sm">
              <div className="bg-gray-50 px-6 py-4 border-b flex justify-between items-center">
                ...
                <div className="bg-green-100 text-green-800 px-4 py-1 rounded-full font-mono font-bold text-lg">
                  Total: {yardTotal}
                </div>
              </div>
                      ...
                      <tr key={cell.id} className="border-b last:border-0 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <input
                            type="number"
                            min="0"
                            className="w-full border-2 border-gray-300 rounded-lg p-2 text-xl font-mono text-center focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all outline-none"
      ...
      {confirmMode && (
        <div className="bg-yellow-50 border border-yellow-200 p-6 rounded-xl shadow-sm">
          <h3 className="text-xl font-bold text-yellow-800 mb-4 flex items-center gap-2">
            <AlertTriangle /> Unlock Summary Confirmation
          </h3>
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="col-span-2 flex justify-between border-t-2 border-gray-800 pt-4 mt-2">
              ...
              <span className="text-2xl font-mono font-bold text-green-600">{totalUnlock}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-4 justify-end pb-12">
        ...
        >
          <Save size={20} />
          {saving ? 'Saving...' : confirmMode ? 'Confirm Unlock' : 'Review & Submit'}
        </button>
      </div>
    </div>
    </PrisonLayout>
